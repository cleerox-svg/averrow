...
| **M10** | "Internal agent names surfaced in customer-side copy" | Was Bundle B; verified `grep` returned no remaining "SPARROW QUEUE" / "Powered by Observer" callsites in `averrow-ui/src`. Re-run when `averrow-tenant` is built — staff-side copy is fine but customer-facing copy should be screened | v3 Phase A skeleton review |
