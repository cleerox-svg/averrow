...
describe('CommandPalette — data search routing', () => {
  ...
  it('routes "View all results" to the bare list route, not a ?q= filtered one', async () => {
    renderWithProviders(<CommandPalette open onClose={vi.fn()} commands={[]} />);
    await typeQuery('ac');
    ...
    // First data-group's "view all" trailer routes to the bare brands list —
    // /brands, never /brands?q=ac (none of the list pages read ?q=, see
    // the DATA_GROUPS comment in CommandPalette.tsx).
    expect(mocks.navigate).toHaveBeenCalledWith('/brands');
  });

  it('renders a "view all" trailer per group, each pointing at its own list route', async () => {
    renderWithProviders(<CommandPalette open onClose={vi.fn()} commands={[]} />);
    await typeQuery('ac');

    const viewAllRows = screen.getAllByText('View all results for “ac”');
    expect(viewAllRows).toHaveLength(4); // one per DATA_GROUPS entry

    await userEvent.click(viewAllRows[1]); // threat actors
    expect(mocks.navigate).toHaveBeenCalledWith('/threat-actors');

    await userEvent.click(viewAllRows[2]); // providers
    expect(mocks.navigate).toHaveBeenLastCalledWith('/providers');

    await userEvent.click(viewAllRows[3]); // campaigns
    expect(mocks.navigate).toHaveBeenLastCalledWith('/campaigns');
  });
